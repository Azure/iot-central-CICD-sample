...
. "$location\PowerShell\IoTC-Helper.ps1"
    ...
    if ($ContentEqual) {
        ...
        $TokensConfig."value" | ForEach-Object {
            ...
            if ($CloudTokens -inotmatch $id) {
                ...
                Set-AzKeyVaultSecret -VaultName $KeyVault -Name $_.id -SecretValue $Secret
                #az keyvault secret set --name $_.id --vault-name $KeyVault --value $Secret
                Write-Host @greenCheck
